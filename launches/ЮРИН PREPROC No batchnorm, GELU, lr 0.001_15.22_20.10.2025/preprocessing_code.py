...
# Source code for save_preprocessing from PLDataModule
    ...
    def save_preprocessing(self, directory_path: str):
        """Saves preprocessing artifacts and source code."""
        ...
        try:
            preprocessing_source = inspect.getsource(self.data_preprocessing)
            code_path = os.path.join(preprocess_dir, "preprocessing_code.py")
            with open(code_path, "w", encoding="utf-8") as f:
                f.write(preprocessing_source)
            print(f"Preprocessing source code saved to: {code_path}")
        except Exception as e:
            print(f"Could not save preprocessing source code: {e}")
