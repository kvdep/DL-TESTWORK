# Source code for data_preprocessing from PLDataModule
    ...
    def data_preprocessing(self, X_train, X_val, X_test, X_true_test):
        ...
        for col in high_card_cols:
            if col in X_train.columns:
                ...
                for val in top_vals:
                    ...
                    X_true_test[f'{col}_{val}'] = (X_true_test[col] == val).astype(int)
        ...
        X_true_test = X_true_test.drop(columns=[col for col in high_card_cols if col in X_true_test.columns])
        ...
        if low_card_cols:
            ...
            X_true_test = pd.get_dummies(X_true_test, columns=low_card_cols, dummy_na=True, dtype=int)
            self.preprocessing_artifacts['low_card_columns'] = low_card_cols

        # --- 4. Надежное выравнивание колонок ---
        train_cols = X_train.columns
        X_val = X_val.reindex(columns=train_cols, fill_value=0)
        X_test = X_test.reindex(columns=train_cols, fill_value=0)
        X_true_test = X_true_test.reindex(columns=train_cols, fill_value=0)
        ...
        if numerical_cols:
            ...
            X_true_test[numerical_cols] = imputer.transform(X_true_test[numerical_cols])
            X_true_test[numerical_cols] = scaler.transform(X_true_test[numerical_cols])
        ...
        return X_train.astype(float).values, X_val.astype(float).values, X_test.astype(float).values, X_true_test.astype(float).values
