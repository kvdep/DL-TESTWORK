...
def run_full_evaluation(model, loader, logger, history, classes, config):
    ...
    # Class Colors: Red, Blue, Yellow, Green
    cls_colors = ['#FF0000', '#0000FF', '#FFD700', '#00AA00']
    ...
    # Header Date
    fig.suptitle(f"Evaluation Report | {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                 fontsize=16, y=1.02, fontweight='light')

    # --- Helper: Gradient Line Plot ---
    def plot_gradient_line(ax, x, y, title, ylabel, rising_is_good=True):
        points = np.array([x, y]).T.reshape(-1, 1, 2)
        segments = np.concatenate([points[:-1], points[1:]], axis=1)

        # Calculate deltas for color
        diffs = np.diff(y)
        if rising_is_good:
            colors = ['#FF0000' if d >= 0 else '#000000' for d in diffs] # Red if up
        else:
            colors = ['#FF0000' if d <= 0 else '#000000' for d in diffs] # Red if down (loss)

        lc = LineCollection(segments, colors=colors, linewidths=2.5)
        ax.add_collection(lc)
        ax.set_xlim(min(x), max(x))
        ax.set_ylim(min(y) - 0.05 * np.ptp(y), max(y) + 0.05 * np.ptp(y))
        ax.set_title(title, fontsize=14)
        ax.set_ylabel(ylabel)
        # Add final value dot
        ax.scatter(x[-1], y[-1], color=colors[-1], s=50, zorder=5)
        ax.text(x[-1], y[-1], f"{y[-1]:.4f}", color=colors[-1], fontweight='bold', ha='left', va='bottom')

    # A. Accuracy (Top Left - Big)
    ax_acc = fig.add_subplot(gs[0, :2])
    epochs = np.arange(1, len(history['val_acc']) + 1)
    plot_gradient_line(ax_acc, epochs, history['val_acc'], 'Validation Accuracy Trend', 'Accuracy', rising_is_good=True)
    ...
    for i in range(n_classes):
        ...
        roc_auc = auc(fpr, tpr)
        ax_roc.plot(fpr, tpr, lw=2, color=cls_colors[i % len(cls_colors)],
                    label=f'{classes[i]} (AUC={roc_auc:.2f})')
    ax_roc.plot([0, 1], [0, 1], 'k--', lw=0.8, alpha=0.5)
    ax_roc.set_title('ROC Curves', fontsize=14)
    ...
    # Custom Black-Red cmap
    cmap_rb = sns.blend_palette(['white', 'black', '#FF0000'], as_cmap=True)
    sns.heatmap(cm, annot=True, fmt='d', cmap=cmap_rb, ax=ax_cm, cbar=False,
                xticklabels=classes, yticklabels=classes, square=True, annot_kws={"size": 12})
    ax_cm.set_title('Confusion Matrix', fontsize=14)
    ax_cm.set_ylabel('Actual')
    ax_cm.set_xlabel('Predicted')

    # D. Precision/Recall/F1 Table (Mid Center - Spanning down)
    ax_metrics = fig.add_subplot(gs[1:3, 1])
    ax_metrics.axis('off')
    rep = classification_report(y_true, y_pred, target_names=classes, output_dict=True)
    df_rep = pd.DataFrame(rep).T.iloc[:-3, :3] # Only classes, no avg

    # Create a visual table
    table_data = []
    for cls in classes:
        row = [cls]
        row.append(f"{rep[cls]['precision']:.3f}")
        row.append(f"{rep[cls]['recall']:.3f}")
        row.append(f"{rep[cls]['f1-score']:.3f}")
        table_data.append(row)

    table = ax_metrics.table(cellText=table_data, colLabels=["Class", "Precision", "Recall", "F1"],
                             loc='center', cellLoc='center', edges='open')
    table.auto_set_font_size(False)
    table.set_fontsize(11)
    table.scale(1, 2)

    # Colorize class names in table
    for i in range(len(classes)):
        cell = table[(i+1, 0)]
        cell.set_text_props(color=cls_colors[i % len(cls_colors)], fontweight='bold')
        for j in range(4):
            table[(i+1, j)].set_edgecolor('lightgray')
            table[(0, j)].set_text_props(fontweight='bold')
    ax_metrics.set_title('Detailed Metrics', fontsize=14)

    # E. Confidence Histogram (Mid Right) - Interpreted as FPR/TPR separation visualization
    ax_hist = fig.add_subplot(gs[1, 2])
    max_probs = np.max(y_prob, axis=1)
    correct_mask = (y_pred == y_true)
    ax_hist.hist(max_probs[correct_mask], bins=20, alpha=0.6, color='#FF0000', label='Correct', density=True)
    ax_hist.hist(max_probs[~correct_mask], bins=20, alpha=0.6, color='black', label='Wrong', density=True)
    ax_hist.set_title('Confidence Distribution', fontsize=14)
    ...
    # Gradient line for loss: Red if decreasing (good), Black if increasing (bad)
    plot_gradient_line(ax_loss, loss_epochs, loss_hist, 'Training Loss', 'Loss', rising_is_good=False)
    ...
    for i in range(n_classes):
        ...
        ax_pr.plot(recall, precision, lw=2, color=cls_colors[i % len(cls_colors)],
    ...
    plt.close(fig)
    return model, loader, classes, history, 0 # Dummy return to match signature expectation
