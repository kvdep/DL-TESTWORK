...
def run_full_evaluation(model, loader, logger, history, classes, config):
    print("Running Full Evaluation & Visualization...")
    model.eval()
    ...
    # --- 2. Setup Aesthetic Layout ---
    sns.set_theme(style="white", font_scale=0.9)
    plt.rcParams.update({'text.color': 'black', 'axes.labelcolor': 'black', 'xtick.color': 'black', 'ytick.color': 'black'})

    fig = plt.figure(figsize=(20, 12), constrained_layout=True)
    fig.patch.set_facecolor('white')
    gs = GridSpec(3, 3, figure=fig, width_ratios=[1, 0.8, 1])

    # Colors
    palette = sns.color_palette("husl", n_classes)
    line_color_train = "#2ecc71" # Emerald
    line_color_val = "#e74c3c"   # Alizarin

    # --- COLUMN 1: Accuracy, Confusion Matrix, Loss ---

    # A. Accuracy Graph (Top Left)
    ax_acc = fig.add_subplot(gs[0, 0])
    epochs = range(1, len(history['val_acc']) + 1)
    sns.lineplot(x=epochs, y=history['val_acc'], ax=ax_acc, color=line_color_val, linewidth=3, marker='o', label='Val Acc')
    ax_acc.set_title("Validation Accuracy Trend", fontweight='bold', fontsize=12)
    ...
    # B. Confusion Matrix (Mid Left)
    ax_cm = fig.add_subplot(gs[1, 0])
    cm = confusion_matrix(y_true, y_pred)
    # Normalize for better color visualization if imbalance exists, but annotate raw
    cm_norm = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
    sns.heatmap(cm_norm, annot=cm, fmt='d', cmap='Blues', ax=ax_cm, cbar=False,
                xticklabels=classes, yticklabels=classes)
    ax_cm.set_title("Confusion Matrix", fontweight='bold', fontsize=12)
    ax_cm.set_ylabel("True Label")
    ax_cm.set_xlabel("Predicted Label")

    # C. Loss Graph (Bottom Left)
    ax_loss = fig.add_subplot(gs[2, 0])
    train_loss = history['train_loss']
    # Handle potential size mismatch if validation ran less frequently, assumes aligned
    sns.lineplot(x=range(1, len(train_loss)+1), y=train_loss, ax=ax_loss, color=line_color_train, linewidth=3, label='Train Loss')
    ax_loss.set_title("Training Loss", fontweight='bold', fontsize=12)
    ax_loss.set_xlabel("Epochs")
    ax_loss.set_ylabel("Loss")
    ax_loss.fill_between(range(1, len(train_loss)+1), train_loss, alpha=0.1, color=line_color_train)
    ax_loss.grid(True, linestyle='--', alpha=0.3)
    ax_loss.spines['top'].set_visible(False)
    ax_loss.spines['right'].set_visible(False)

    # --- COLUMN 2: Metrics Heatmap (Spanning Vertically) ---

    # D. Classification Report Heatmap
    ax_metrics = fig.add_subplot(gs[:, 1])
    report = classification_report(y_true, y_pred, target_names=classes, output_dict=True)
    df_rep = pd.DataFrame(report).T.iloc[:-3, :3] # Exclude avg/accuracy, keep Prec/Rec/F1

    sns.heatmap(df_rep, annot=True, fmt='.2f', cmap='RdYlGn', center=0.8, ax=ax_metrics,
                cbar=False, annot_kws={"size": 12, "weight": "bold"})
    ax_metrics.set_title("Class-wise Metrics (P/R/F1)", fontweight='bold', fontsize=14)
    ax_metrics.set_yticklabels(df_rep.index, rotation=0, fontweight='bold')
    ax_metrics.tick_params(axis='x', top=True, labeltop=True, bottom=False, labelbottom=False)

    # --- COLUMN 3: ROC Breakdown ---

    y_bin = label_binarize(y_true, classes=range(n_classes))

    # E. ROC-AUC Scores Bar Chart (Top Right)
    ...
    sns.barplot(x=auc_scores, y=classes, palette=palette, ax=ax_auc, orient='h')
    ax_auc.set_xlim(0, 1.05)
    ax_auc.set_title("ROC-AUC Score by Class", fontweight='bold', fontsize=12)
    ax_auc.bar_label(ax_auc.containers[0], fmt='%.2f', padding=3)
    ax_auc.spines['right'].set_visible(False)
    ax_auc.spines['bottom'].set_visible(False)
    ax_auc.set_xticks([])

    # F. ROC Curves / FPR-TPR (Mid Right)
    ax_roc = fig.add_subplot(gs[1, 2])
    for i in range(n_classes):
        fpr, tpr, _ = roc_curve(y_bin[:, i], y_prob[:, i])
        ax_roc.plot(fpr, tpr, lw=2, color=palette[i], alpha=0.8, label=classes[i])

    ax_roc.plot([0, 1], [0, 1], 'k--', lw=1, alpha=0.5)
    ax_roc.set_title("ROC Curves (FPR vs TPR)", fontweight='bold', fontsize=12)
    ...
    for i in range(n_classes):
        ...
        ax_pr.plot(r, p, lw=2, color=palette[i], alpha=0.8, label=f'{classes[i]} (AP={ap:.2f})')

    ax_pr.set_title("Precision-Recall Curves", fontweight='bold', fontsize=12)
    ax_pr.set_xlabel("Recall")
    ax_pr.set_ylabel("Precision")
    ax_pr.grid(True, alpha=0.2)
    ax_pr.legend(loc='lower left', fontsize='x-small', frameon=True)

    # --- Save & Return ---
    # Global title
    fig.suptitle(f"Model Evaluation Report | Best Acc: {max(history['val_acc']):.4f}",
                 fontsize=18, fontweight='heavy', y=1.02)

    save_path = "evaluation_report.png"
    if logger and logger.current_exp_folder:
         save_path = f"{logger.current_exp_folder}/evaluation_report.png"

    fig.savefig(save_path, dpi=150, bbox_inches='tight')
    print(f"[INFO] Report saved to {save_path}")
    plt.close(fig)

    return model, loader, classes, history, max(history['val_acc'])
