# RAW EXECUTION HISTORY - 2025-12-16 18:08:22.111726
...
class GitLogger:
    ...
    def save_code(self):
        ...
        # 1. Recover Imports from History (What you actually typed)
        raw_history = globals().get('_ih', [])
        imports = set()
        for code in raw_history:
            # Simple parsing to find import lines
            for line in code.split('\n'):
                line = line.strip()
                if (line.startswith('import ') or line.startswith('from ')) and ':' not in line:
                    clean_line = line.split('#')[0].strip()
                    if clean_line: imports.add(clean_line)
        
        lines.extend(sorted(list(imports)))
        lines.append("\n# --- Definitions ---\n")

        # 2. Recover Functions, Classes, and Config from Memory (Current State)
        for name, obj in globals().items():
            if name.startswith('_') or name in ['In', 'Out', 'exit', 'quit', 'get_ipython']: continue
            
            # Functions & Classes (Only those defined in this notebook)
            if (inspect.isfunction(obj) or inspect.isclass(obj)) and hasattr(obj, '__module__') and obj.__module__ == '__main__':
                try:
            ...
            # CONFIG Dictionary
                ...
                lines.append(f"CONFIG = {pprint.pformat(obj)}\n")
...
class GitLogger:
    ...
    def save_code(self):
        ...
        # 1. Recover Imports from History
        raw_history = globals().get('_ih', [])
        imports = set()
        for code in raw_history:
            for line in code.split('\n'):
        ...
        # 2. Recover Functions, Classes, and Config
        for name, obj in globals().items():
            if name.startswith('_') or name in ['In', 'Out', 'exit', 'quit', 'get_ipython']: continue
            ...
            # CONFIG Dictionary (Sanitized)
            elif name == 'CONFIG':
                safe_config = obj.copy() if isinstance(obj, dict) else obj
                if isinstance(safe_config, dict) and 'token' in safe_config:
                    safe_config['token'] = "REDACTED"
                lines.append(f"CONFIG = {pprint.pformat(safe_config)}\n")
...
class GitLogger:
    ...
    def save_code(self):
        ...
        # 1. Grab Raw Input History (Exactly what you typed)
        # This prevents expansion of f-strings and keeps variables as code
        raw_inputs = globals().get('_ih', [])
        
        # 2. Safety: Identify token to Redact
        # (Even if hardcoded in a cell, we must mask it to pass GitHub security)
        try:
            # self.auth_url structure is https://TOKEN@github...
            secret = self.auth_url.split('//')[1].split('@')[0]
        except:
            secret = "IMPOSSIBLE_TOKEN_STRING"

        with open(target, 'w', encoding='utf-8') as f:
            f.write(f"# RAW EXECUTION HISTORY - {datetime.now()}\n\n")
            
            for i, cell in enumerate(raw_inputs):
                if not cell.strip(): continue
                
                # Check for secret in the raw text and redact
                if secret in cell and len(secret) > 8:
                    cell = cell.replace(secret, "REDACTED_SECRET")
                
                f.write(f"# --- Cell {i} ---\n")
                f.write(cell)
                f.write("\n\n")
                
        print(f"[INFO] Raw code saved to {target}")
