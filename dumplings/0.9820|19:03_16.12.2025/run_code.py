...
def get_dataloaders():
    ...
    train_tf = transforms.Compose([
        # Исправляем проблему с палитрой/прозрачностью (предупреждение в логах)
        transforms.Lambda(lambda x: x.convert('RGB')),

        # Вместо обычного Resize используем RandomResizedCrop.
        # Это лучшая аугментация для обучения: заставляет модель смотреть на детали, а не только на форму.
        transforms.RandomResizedCrop(img_size, scale=(0.6, 1.0)),

        # Геометрические искажения
        transforms.RandomHorizontalFlip(p=0.5), # Зеркальное отражение
        transforms.RandomRotation(degrees=15),  # Поворот +/- 15 градусов
        # transforms.RandomVerticalFlip(p=0.5), # Можно добавить, если пельмени сняты сверху
        ...
        # Нормализация (как у вас)
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),

        # RandomErasing - вырезает случайные квадраты (заставляет модель восстанавливать детали)
        # Ставить ПОСЛЕ ToTensor
        transforms.RandomErasing(p=0.2, scale=(0.02, 0.15))
    ])

    # 2. Аугментации для ВАЛИДАЦИИ (Только ресайз и формат)
    val_tf = transforms.Compose([
        transforms.Lambda(lambda x: x.convert('RGB')),

        # Для валидации лучше делать Resize чуть больше, а потом CenterCrop.
        # Это стандартный подход (например, ресайз до 360, кроп 338).
        # Но можно оставить просто Resize, если хотите проще.
        transforms.Resize((img_size, img_size)),
        # Альтернативный вариант для повышения точности:
